...
@router.post("", response_model=UserOut, status_code=201)
def create_user(user_info: UserCreate):
    # duplicate check
    existing = (
        supabase.table("users").select("id").eq("email", user_info.email).execute()
    )
    if existing.data:
        raise HTTPException(
            status_code=409, detail="check your email. already exist email."
        )

    # DB Insert
    result = (
        supabase.table("users")
        .insert({"email": user_info.email, "username": user_info.username})
        .execute()
    )
...
def update_user(user_id: UUID, userinfo: UserUpdate):
    result = (
        supabase.table("users")
